...
export const validateDateAndTime = (
  ...
  const requiredStartDate = DateTime.fromMillis(
    requiredDateAndTime.startDate,
  ).startOf("day");
  ...
  if (
    nowDateOnly < requiredStartDate ||
    (requiredEndDate && nowDateOnly > requiredEndDate)
  ) {
    return false;
  }
...
export const validateDrop = ({
  ...
  try {
    ...
    const isEventTicket = allTicketOptions.find(
      (option) => option.drop_id === dropId,
    );
    if (dropId === "" || !isEventTicket) {
      return {
        status: "error",
        message: "Ticket does not exist for this event.",
      };
    }
    ...
    if (ticketToVerify !== dropId) {
      return {
        status: "error",
        message:
          "Ticket does not match the selected ticket type but is valid for the event.",
      };
    }
