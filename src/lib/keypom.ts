...
class KeypomJS {
  ...
  GetMarketListings = async ({
    ...
    try {
      ...
      if (
        ...
        this.allEventsGallery.length !== totalSupply
      ) {
      ...
      throw new Error(
        "Failed to get all market listings due to error: " + String(error),
      );
    }
  };
  ...
  verifyDrop = async (contractId: string) => {
    ...
    if (
      ...
      throw new Error(
        "Please supply supportedKeypomContracts, networkId and contractId",
      );
    }

    if (supportedKeypomContracts[networkId][contractId] === undefined) {
      throw new Error(
        "Linkdrop is invalid and isn't officially supported by Keypom contract.",
      );
    }
  ...
  getCurrentKeyOwner = async (publicKey: string) => {
  ...
  /*
    ...
    simple -> simple drop?
  */
  getLinkdropType = async (drop: ProtocolReturnedDrop, contractId: string) => {
    await this.verifyDrop(contractId);
  ...
  getAllDrops = async ({ accountId }: { accountId: string }) => {
    try {
      ...
      if (
        this.dropStore[accountId] == null ||
        this.dropStore[accountId] === undefined
      ) {
  ...
  groupAllDropsForAccount = async ({ accountId }: { accountId: string }) => {
    ...
    allDrops = allDrops.filter((drop) => {
      ...
      return isValidTicketNFTMetadata(
  ...
  getPaginatedKeysForTicket = async ({
    ...
    try {
      ...
      const endIndex = Math.min(
        start + limit,
        this.purchasedTicketsById[dropId].totalKeys,
      );
      ...
      for (let i = start; i < endIndex; i += KEY_ITEMS_PER_QUERY) {
        ...
        if (
          ...
          for (let j = 0; j < fetchedKeys.length; j++) {
            this.purchasedTicketsById[dropId].dropKeyItems[batchStart + j] =
              fetchedKeys[j];
          }
        }
      }
      ...
      // Return the requested slice from the cache
      return this.purchasedTicketsById[dropId].dropKeyItems.slice(
        start,
        endIndex,
      );
    } catch (e) {
  ...
  getDropSupplyForOwner = async ({ accountId }) =>
    await getDropSupplyForOwner({ accountId });
  ...
  getDropData = async ({
    ...
    if (type === DROP_TYPE.NFT) {
      ...
      try {
        ...
        const { nftData } = await this.getNFTorTokensMetadata(
          fcMethods[0][0],
          drop.drop_id,
        );
  ...
  deleteDrops = async ({ wallet, dropIds }) =>
    await deleteDrops({ wallet, dropIds });
  ...
  getAvailableKeys = async (dropId: string) =>
    await getKeySupplyForDrop({ dropId });
  ...
  getTokenClaimInformation = async (contractId: string, secretKey: string) => {
    ...
    const linkdropType = await this.getLinkdropType(drop, contractId);
  ...
  getNFTClaimInformation = async (contractId: string, secretKey: string) => {
    ...
    if (linkdropType !== DROP_TYPE.NFT) {
      throw new Error(
        "This drop is not an NFT drop. Please contact your drop creator.",
      );
    }
  ...
  getTicketNftInformation = async (contractId: string, secretKey: string) => {
    ...
    if (linkdropType !== DROP_TYPE.TICKET) {
      throw new Error(
        "This drop is not a Ticket drop. Please contact your drop creator.",
      );
    }
