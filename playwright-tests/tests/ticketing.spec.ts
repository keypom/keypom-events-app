    ...
    test.beforeEach(async ({ page }) => {
      ...
      await mockRpcRequest({
        ...
        modifyOriginalResultFunction: (result) => {
          ...
          result.drop_config.nft_keys_config.token_metadata.extra = JSON.stringify(
            { "eventId": EVENT_ID }
          );
        ...
        modifyOriginalResultFunction: (result) => {
          result.metadata = JSON.stringify({
            [EVENT_ID]: {
              ...
              "artwork": "https://cloudflare-ipfs.com/ipfs/bafybeibadywqnworqo5azj4rume54j5wuqgphljds7haxdf2kc45ytewpy"
            }
          });
      ...
      await page.goto(`/tickets/ticket/ga_pass#${UNSCANNED_TICKET_PRIVATE_KEY}`);
    });

    test("should show the ticket QR page", async ({ page }) => {
      const QRPageTitle = await page.getByText("You're attending Redacted 2025");
      await expect(QRPageTitle).toBeVisible();
    });

    test("should navigate to welcome page after successful scan", async ({ page }) => {
      await mockRpcRequest({
        page,
        filterParams: {
          method_name: "get_key_information",
